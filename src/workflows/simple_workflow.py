...
class SimpleWorkflow:
    ...
    async def run(self, input_data: WorkflowInput) -> WorkflowResult:
        ...
        """
        try:
            self.logger.info(f"Starting simple workflow for request {input_data.request_id}")
            
            # Execute workflow steps
            result = await self._execute_workflow_steps(input_data)
            
            self.logger.info(f"Simple workflow completed successfully for {input_data.request_id}")
            return WorkflowResult(success=True, result_data=result)
            
        except Exception as e:
            self.logger.error(f"Simple workflow failed for {input_data.request_id}: {e}")
    ...
    async def _execute_workflow_steps(self, input_data: WorkflowInput) -> Dict[str, Any]:
        ...
        if not validation_result.get("valid", False):
            raise ValueError(f"Input validation failed: {validation_result.get('message', 'Unknown error')}")
